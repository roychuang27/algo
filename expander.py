...
class Expander:
    ...
    def __init__(self, lib_paths: List[Path], *, markers: bool = False):
    ...
    def expand_acl(self, acl_file_path: Path) -> List[str]:
        ...
        for line in acl_source.splitlines():
            ...
            if not m:
                alias_only = False
                break
            if alias_include is None:
                alias_include = m
            else:
                alias_only = False
                break
        if alias_only and alias_include and alias_include.group(1).endswith(".hpp"):
            return self.expand_acl(self.find_acl(alias_include.group(1)))

        result = []  # type: List[str]
        if self.markers:
            result.append("// BEGIN {}".format(acl_file_path))
        for line in acl_source.splitlines():
            if self.is_ignored_line(line):
                continue
            ...
            result.append(line)
        if self.markers:
            result.append("// END {}".format(acl_file_path))
        return result
...
if __name__ == "__main__":
    ...
    parser.add_argument("--markers", action="store_true", help="Emit BEGIN/END markers")
    ...
    expander = Expander(lib_paths, markers=opts.markers)
